...
public class ADD_CAR{
	...
	/*@ public normal_behavior
		requires true;
 		assignable \nothing;
		ensures \result <==> (machine.OBJECTS.difference(machine.get_objects()).has(Car) && NAT.instance.has(Desc) && NAT.instance.has(W) && NAT.instance.has(H) && INT.instance.has(X) && INT.instance.has(Y) &&  (\forall Integer lane;((machine.get_lanes().has(lane)) ==> ((X).compareTo(machine.get_left_border().apply(lane)) > 0 && (X).compareTo(machine.get_right_border().apply(lane)) < 0))) &&  (\forall Integer lane;((machine.get_lanes().has(lane)) ==> ((Y).compareTo(machine.get_finish_line().apply(lane)) < 0))) &&  (\forall Integer Obj1;((machine.get_objects().has(Obj1)) ==> (!((new Integer(machine.get_posX().apply(Obj1) - X)).compareTo(new Integer(new Integer(machine.get_width().apply(Obj1) / new Integer(2)) + new Integer(W / new Integer(2)))) < 0) && (new Integer(machine.get_posY().apply(Obj1) - Y)).compareTo(new Integer(new Integer(machine.get_height().apply(Obj1) / new Integer(2)) + new Integer(H / new Integer(2)))) < 0))) && NAT.instance.has(M) && NAT.instance.has(F)); */
	public /*@ pure */ boolean guard_ADD_CAR( Integer Car, Integer Desc, Integer H, Integer W, Integer X, Integer Y, Integer F, Integer M) {
		return (machine.OBJECTS.difference(machine.get_objects()).has(Car) && NAT.instance.has(Desc) && NAT.instance.has(W) && NAT.instance.has(H) && INT.instance.has(X) && INT.instance.has(Y) && true && true && true && NAT.instance.has(M) && NAT.instance.has(F));
	}
